...
from textTo3DModelGen.pipeline.stage_05_training_model import TrainingModelPipeline
...
parser.add_argument("--stage", required=True, help="Specify the stage to run: \n1: run data ingestion pipeline.\n2: run data rendering pipeline.\n3: run text embedding pipeline.\
                    \n3: run data split pipeline.\nn5: run training model pipeline.\n6: run all pipeline.")
...
if __name__ == "__main__":
    if args.stage == "1":
        ...
        print(f"{args.stage} is an invalid stage, should be in [1,2,3,4,5,6]")
